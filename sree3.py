...
def new_user():      #data = request.json.get()
    ...
    if Sree.query.filter_by(username=username).first() is not None:     #abort(400), For Existing User
...
def verify_password(username, password):
    user = Sree .query.filter_by(username=username).first()  #Finds the user by user name
